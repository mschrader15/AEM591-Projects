...
class SigmaPoints:
    ...
    def __init__(
        ...
        c = 0.5 / (self.dim + self._lambda)

        self.Wc = np.full(self.size, c)
        self.Wm = np.full(self.size, c)
        self.Wc[0] = self._lambda / (self.dim + self._lambda) + (
            1 - self.a ** 2 + self.b
        )
...
class UKF(BaseFilter):
    ...
    def _predict(
        ...
        x, P_priori = self._unscented_transform(
            sigma_f, angle_ind=(2,), noise_cov=self.Q
        )
    ...
    def _update(self, measurement: np.ndarray, *args, **kwargs) -> None:
        ...
        zp, S = self._unscented_transform(
            sigmas_h, angle_ind=(0, 1, 2), noise_cov=self.R
        )

        Pxz = self.cross_variance(self.x_priori, zp, self.sigmas_f, sigmas_h)
        ...
        self.P_posteriori = self.P_priori - np.dot(self.K, np.dot(S, self.K.T))
    ...
    def _unscented_transform(
        self, sigma: np.ndarray, angle_ind: tuple, noise_cov: np.ndarray = None
    ) -> Tuple[np.ndarray,]:
    ...
    def cross_variance(self, x, z, sigmas_f, sigmas_h):
        ...
        for i in range(sigmas_f.shape[0]):
            Pxz += self.sp.Wc[i] * np.outer(
                np.subtract(sigmas_f[i], x), np.subtract(sigmas_h[i], z)
            )
...
if __name__ == "__main__":
    ...
    ukf.P_posteriori = np.diag([0.1, 0.1, 0.1])
